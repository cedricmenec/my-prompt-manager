...
export function SidebarNav() {
  const { state, dispatch, activeFilter, setActiveFilter, appView } = usePrompts()
  const [settingsPanelOpen, setSettingsPanelOpen] = useState(false)

  // Uncollected count (prompts with no tags)
  const uncollectedCount = state.prompts.filter(
    (p) => !p.tags || p.tags.length === 0,
  ).length

  // All-prompts collections (for prompts mode)
  const allCollectionsMap = new Map<string, number>()
  for (const prompt of state.prompts) {
    for (const tag of prompt.tags ?? []) {
      allCollectionsMap.set(tag, (allCollectionsMap.get(tag) ?? 0) + 1)
    }
  }
  const allCollections = Array.from(allCollectionsMap.entries()).sort(([a], [b]) =>
    a.localeCompare(b),
  )

  // Gallery collections (only tags from image prompts)
  const imageCollectionsMap = new Map<string, number>()
  for (const prompt of state.prompts.filter((p) => p.type === 'image')) {
    for (const tag of prompt.tags ?? []) {
      imageCollectionsMap.set(tag, (imageCollectionsMap.get(tag) ?? 0) + 1)
    }
  }
  const imageCollections = Array.from(imageCollectionsMap.entries()).sort(([a], [b]) =>
    a.localeCompare(b),
  )

  const collections = appView === 'gallery' ? imageCollections : allCollections

  // Determine whether a nav item is currently active
  function isActive(id: string): boolean {
    if (id === 'all-prompts' || id === 'all-images') return activeFilter.type === 'all'
  ...
  return (
    // Fixed sidebar container: 260px wide, full height (from parent flex), left-anchored
    <aside className="flex h-full w-[260px] shrink-0 flex-col border-r border-border bg-surface">
      {/* CTA button — mode-aware */}
      <div className="px-3 py-3">
        {appView === 'gallery' ? (
          <button
            onClick={() => dispatch({ type: 'OPEN_CREATE', initialType: 'image' })}
      ...
      {/* Primary nav links — mode-aware */}
      <nav className="flex flex-col gap-0.5 px-3" aria-label="Primary navigation">
        {appView === 'gallery' ? (
          <>
            <button className={navLinkClass('all-images')} onClick={() => setActiveFilter({ type: 'all' })}>
              <svg
