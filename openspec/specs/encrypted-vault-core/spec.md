...
Provides the framework-agnostic core of the `@byo-prompt/encrypted-vault` SDK: cryptographic primitives (PBKDF2 + AES-256-GCM), session cache with configurable TTL, storage abstraction interface, and vault lifecycle orchestration — all with zero UI framework dependencies.
