...
Defines the lifecycle for optimized local reference image assets attached to image prompts. Assets are processed in the browser, stored locally in IndexedDB as WebP `Blob` records, and rendered through object URLs before falling back to legacy remote `imageUrl` references.
