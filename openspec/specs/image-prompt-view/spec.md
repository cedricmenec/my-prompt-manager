...
Defines the dedicated detail view for image-generation prompts, shown when a prompt with `type === 'image'` is selected in Gallery mode. Displays a local image asset or remote image prominently at the top, followed by prompt metadata, and provides Back and Edit actions.

## Requirements

### Requirement: Image prompt view displays prompt image prominently
The system SHALL provide an `ImagePromptView` component that is rendered when the user selects a prompt with `type === 'image'` while in Gallery mode (`appView === 'gallery'`). The view SHALL display the local image asset referenced by `imageAssetId` when available, otherwise the prompt's `imageUrl`, at large size (max height approximately 60% of the viewport height) with `object-contain` scaling, above the prompt text content and metadata. If neither image reference is available, a placeholder SHALL be shown in place of the image.
...
#### Scenario: Placeholder shown when no image is available
