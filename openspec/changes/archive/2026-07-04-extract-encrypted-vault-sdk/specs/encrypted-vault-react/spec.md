...
Provides optional React hooks and generic UI components for the local-encrypted-vault SDK. Includes `useVault()` hook for state management, `VaultGate` for guarding application rendering, and modal components for vault creation, unlock, and settings.
