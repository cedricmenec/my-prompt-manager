...
**Reason**: The filter buttons ("Tags", "Language", "Favorites") were rendered but non-functional and add visual noise. They are deferred until a dedicated filtering capability is implemented.
**Migration**: Filtering will be reintroduced as a new capability when the filter feature is specified. The view-toggle is now part of the new "View toggle in top app bar" requirement above.
